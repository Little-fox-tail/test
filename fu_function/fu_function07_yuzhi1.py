...
def process_folder(folder_path, save_path):
    ...
    if len(images) > 0:
        ...
        fusion_result = cap_pixel_values(fusion_result)  # 将大于120的像素值设置为255

        folder_name = os.path.basename(folder_path)
        filename = f'exposure_fusion_result_{folder_name}_{threshold}.bmp'
        cv2.imwrite(os.path.join(save_path, filename), fusion_result)  # 保存图像

if __name__ == '__main__':
    save_path = 'result/result_main07/duobaoguang03_test'
