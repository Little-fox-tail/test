...
def exposure_fusion(images):
    if not images:
        raise ValueError("图像列表为空，无法进行曝光融合。")
    
    # 将图像转换为灰度以计算亮度
    gray_images = [cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) for img in images]
    
    # 计算每个图像的亮度（平均灰度值）
    brightness = [np.mean(gray) for gray in gray_images]
    ...
    # 计算每个图像的权重
    weights = [(b - min_brightness) / (max_brightness - min_brightness) for b in brightness]
    
    # 确保权重的总和为1
    weights = np.array(weights) / sum(weights)
    
    # 计算加权平均图像
    fusion_result = np.zeros_like(images[0]).astype(np.float32)
    for img, weight in zip(images, weights):
        fusion_result += img.astype(np.float32) * weight
...
def calculate_dynamic_threshold(image, lower=80, upper=170, smoothing_window=5):
    ...
    2. 检测平滑后的直方图中的局部最小值和局部最大值，并选择满足两侧均有局部最大值的局部最小值作为初步阈值；
