...
# 将图像四周边缘转成255
def set_edges_to_255(image, edge_width=100):# 边缘宽度
    height, width = image.shape[:2]
    # 设置顶部和底部边缘
    image[:edge_width, :] = 190
    image[-edge_width:, :] = 180
    # 设置左侧和右侧边缘
    image[:, :edge_width] = 195
    image[:, -edge_width:] = 185
    return image
...
def process_images(base_path, sub_folders_dof, sub_folders_us, save_path):
    ...
    for image_path in image_files:
        ...
        # # 保存结果
        # save_file = os.path.join(save_path, f"fu_{os.path.splitext(image_name)[0]}_medianBlur.Bmp")
        # cv2.imwrite(save_file, fusion_result)
