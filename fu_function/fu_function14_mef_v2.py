...
import glob
# 导入fu_base模块
from fu_base import fu_base_pyramid as fbpy
from fu_base import fu_base_reconstruct as fbre
from fu_base import fu_base_weight as fbwe
...
# 曝光融合
def mertens_fusion(images, contrast_weight=0.2, saturation_weight=1.0, exposure_weight=0.2, levels=5):
    # 计算权重图
    weights = fbwe.compute_weights(
        images, 
        contrast_weight, 
        saturation_weight, 
        exposure_weight
    ...
    # 融合金字塔
    blended_pyramid = fbpy.blend_pyramids(images_float, weights, levels)

    # 重建图像
    fused = fbre.reconstruct_image(blended_pyramid)
...
# 将图像四周宽度为20像素的边缘转成255
def set_edges_to_255(image, edge_width=20):
...
def process_images(base_path, sub_folders_dof, sub_folders_us, save_path):
    ...
    for image_path in image_files:
        ...
        fusion_result = mertens_fusion(aligned_images)
        ...
        # 新增：亮度反转（255 - 像素值）
        fusion_result = 255 - fusion_result  # 直接反转亮度

        # 先使用中值滤波去除斑点
        fusion_result = cv2.medianBlur(fusion_result, 3)
        save_file = os.path.join(save_path, f"fusion_{os.path.splitext(image_name)[0]}_medianBlur.Bmp")
        cv2.imwrite(save_file, fusion_result)
        ...
        # 保存结果
        save_file = os.path.join(save_path, f"fu_{os.path.splitext(image_name)[0]}_{threshold}.Bmp")
        cv2.imwrite(save_file, fusion_result)
